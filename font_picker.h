...
static void fp_render_gl(FpCtx *ctx) {
    ...
    glBegin(GL_LINES);
    glVertex2f(16.0f, LIST_BOT + 2.0f);
    glVertex2f(W - 16.0f, LIST_BOT + 2.0f);
    glEnd();
...
static void fp_draw_layered(void) {
    ...
    for (int y = 0; y < H; y++)
        for (int x = 0; x < W; x++) {
            float fx = x + 0.5f, fy = y + 0.5f, ccx = fr, ccy = fr;
            BOOL in = FALSE;
            if (fx < fr && fy < fr) {
                ccx = fr;
                ccy = fr;
            ...
            if (edge >= 0.5f) { bits[y * W + x] = 0; } else if (edge > -0.5f) {
                float t = (0.5f - edge) * (FP_ALPHA / 255.0f);
                DWORD c = bits[y * W + x];
                bits[y * W + x] =
        ...
        case WM_LBUTTONDOWN: {
            ...
            if (fp_hit_scrollbar(cx, cy)) {
                ...
                if (fp_hit_thumb(ctx, cy, &th_y)) {
                    ...
                    const float LIST_H = (float) (FP_H - FP_FOOTER_H - FP_LIST_TOP_Y);
                    float th_h;
                    fp_sb_thumb(ctx, LIST_TOP, LIST_H,NULL, &th_h);
        ...
        case WM_KEYDOWN: {
            ...
            switch (wParam) {
                case VK_ESCAPE: DestroyWindow(hWnd);
                    return 0;
                case VK_RETURN: fp_confirm(hWnd);
                    return 0;
                case VK_UP:
                case VK_DOWN:
...
static void fp_register(HINSTANCE hi) {
    ...
    a.hInstance = hi;
    a.lpszClassName = "PXVYFontPickerGL";
    RegisterClassA(&a);
    WNDCLASSW w = {0};
    w.lpfnWndProc = FpWndProc;
    w.hInstance = hi;
...
static void show_font_picker(HWND owner, const char *current,
    ...
    if (g_fp_hwnd && IsWindow(g_fp_hwnd)) {
    ...
    g_fp_gl_hwnd = CreateWindowW(L"PXVYFontPickerGL", NULL, WS_POPUP,
...
#endif // RLAQHADMLGPEJWNDQHRQKDWLAOZMFH_FONT_PICKER_H
