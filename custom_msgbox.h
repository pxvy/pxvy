...
static GLuint mb_load_sym_tex(void) {
    ...
    stbir_resize_uint8_linear(src, sw, sh, 0,
                               dst, MB_SYM_SZ, MB_SYM_SZ, 0, STBIR_RGBA);
    stbi_image_free(src);
    ...
    for (int y = 0; y < H; y++)
        for (int x = 0; x < W; x++) {
            ...
            if (edge >= 0.5f) { bits[y * W + x] = 0; } else if (edge > -0.5f) {
                float t = (0.5f - edge) * (MB_ALPHA / 255.0f);
                DWORD c = bits[y * W + x];
                bits[y * W + x] =
    ...
    switch (msg) {
        ...
        case WM_KEYDOWN:
            switch (wParam) {
                case VK_ESCAPE: {
                    MbCtx *ctx = (MbCtx *) GetWindowLongPtr(hWnd, 0);
                    MbCallback cb = ctx ? ctx->callback : NULL;
                    void *ud = ctx ? ctx->userdata : NULL;
                    DestroyWindow(hWnd);
...
static void mb_register(HINSTANCE hi) {
    ...
    a.hInstance = hi;
    a.lpszClassName = "PXVYMsgBoxGL";
    RegisterClassA(&a);
    WNDCLASSW w = {0};
    w.lpfnWndProc = MbWndProc;
    w.hInstance = hi;
...
static void show_msgbox(HWND owner,
                        ...
                        MbCallback callback,
                        void *userdata) {
    if (g_mb_hwnd && IsWindow(g_mb_hwnd)) {
